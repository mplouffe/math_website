...
var displayPythagorasOnUnitCircle = function(){
    ...
    context.fillText("Θ", 128+18, 128-3);
}

var displayTrigFunctions = function(){
    let canvas = document.getElementById("unitCircleViz_04");
    canvas.width = 256;
    canvas.height = 256;
    let context = canvas.getContext('2d');
    ...
    context.moveTo(128,128);
    context.lineTo(128+(64*Math.cos(Math.PI/4)), 128-(64*Math.sin(Math.PI/4)));
    context.arc(128+(64*Math.cos(Math.PI/4)), 128-(64*Math.sin(Math.PI/4)), 2, 0, 2 * Math.PI, true);;
    // context.lineTo(128+(64*Math.cos(Math.PI/4)), 128);
    ...
    context.fillStyle = "#000";
    context.fillText("(Cos(Θ), Sin(Θ))", 128 + (64*Math.cos(Math.PI/4)) + 2, 128-(64*Math.sin(Math.PI/4)) - 5);
    context.fillText("Θ", 128+18, 128-3);
}
